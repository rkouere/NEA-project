...
def extract_glimpse_v2(input, size, offsets, centered=True, normalized=True, uniform_noise=True, noise="uniform", name=None):
  ...
  * If the coordinates are not normalized they are interpreted as
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "ExtractGlimpseV2", name,
        tld.op_callbacks, input, size, offsets, "centered", centered,
        "normalized", normalized, "uniform_noise", uniform_noise, "noise",
        noise)
      return _result
...
def image_projective_transform_v2(images, transforms, output_shape, interpolation, fill_mode="CONSTANT", name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "ImageProjectiveTransformV2",
        name, tld.op_callbacks, images, transforms, output_shape,
...
def image_projective_transform_v2_eager_fallback(images, transforms, output_shape, interpolation, fill_mode, name, ctx):
  ...
  if fill_mode is None:
    fill_mode = "CONSTANT"
  fill_mode = _execute.make_str(fill_mode, "fill_mode")
  _attr_dtype, (images,) = _execute.args_to_matching_eager([images], ctx)
