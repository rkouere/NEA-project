  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        ...
        "StatelessParameterizedTruncatedNormal", name, tld.op_callbacks,
        shape, seed, means, stddevs, minvals, maxvals)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
    except _core._FallbackException:
...
def stateless_parameterized_truncated_normal_eager_fallback(shape, seed, means, stddevs, minvals, maxvals, name, ctx):
  ...
  _attr_Tseed, (seed,) = _execute.args_to_matching_eager([seed], ctx, _dtypes.int64)
  _attr_dtype, _inputs_dtype = _execute.args_to_matching_eager([means, stddevs, minvals, maxvals], ctx)
...
def stateless_random_binomial(shape, seed, counts, probs, dtype=_dtypes.int64, name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "StatelessRandomBinomial",
        name, tld.op_callbacks, shape, seed, counts, probs, "dtype", dtype)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
    except _core._FallbackException:
...
def stateless_random_gamma_v2(shape, seed, alpha, name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "StatelessRandomGammaV2", name,
        tld.op_callbacks, shape, seed, alpha)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
    except _core._FallbackException:
...
def stateless_random_gamma_v2_eager_fallback(shape, seed, alpha, name, ctx):
  _attr_dtype, (alpha,) = _execute.args_to_matching_eager([alpha], ctx)
...
def stateless_random_poisson(shape, seed, lam, dtype, name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "StatelessRandomPoisson", name,
        tld.op_callbacks, shape, seed, lam, "dtype", dtype)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
    except _core._FallbackException:
...
def stateless_random_poisson_eager_fallback(shape, seed, lam, dtype, name, ctx):
  dtype = _execute.make_type(dtype, "dtype")
  _attr_Rtype, (lam,) = _execute.args_to_matching_eager([lam], ctx)
  _attr_T, (shape,) = _execute.args_to_matching_eager([shape], ctx)
  _attr_Tseed, (seed,) = _execute.args_to_matching_eager([seed], ctx, _dtypes.int64)
...
def stateless_random_uniform_full_int(shape, seed, dtype=_dtypes.uint64, name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name,
        "StatelessRandomUniformFullInt", name, tld.op_callbacks, shape, seed,
        "dtype", dtype)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
