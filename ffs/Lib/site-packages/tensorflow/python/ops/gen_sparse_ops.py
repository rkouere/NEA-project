...
def sparse_cross_hashed(indices, values, shapes, dense_inputs, num_buckets, strong_hash, salt, name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "SparseCrossHashed", name,
        tld.op_callbacks, indices, values, shapes, dense_inputs, num_buckets,
        strong_hash, salt)
...
def sparse_cross_v2(indices, values, shapes, dense_inputs, sep, name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "SparseCrossV2", name,
        tld.op_callbacks, indices, values, shapes, dense_inputs, sep)
